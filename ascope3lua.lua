...
-- V27: Tied O_ECG_BEAT_OUT directly to visual drawing of heartbeat peak in onDraw. (Incorrectly)
-- V26: Adjusted peak trigger condition in onDraw to prevent occasional visual skips.
-- V25: Changed all block comments (version history) to single-line comments.
...
-- Tick
function onTick()
    g_t=g_t+1;local bIncMx=igB(I_BTN_INC_MX);local bDecMx=igB(I_BTN_DEC_MX);local bDecMn=igB(I_BTN_DEC_MN)
    ...
    if bPwrP and not g_prv_b[I_BTN_PWR] then
        ...
        if not g_pwr then -- Reset states if powered off
            g_trks={};g_fcs_trk=nil;g_lck_st=0;g_srch_dir=1;g_d_t2="C:---";g_d_t3="A:---";
    ...
    if not g_pwr then
        ...
        -- g_d_t1, g_d_t2, g_d_t3, g_d_t4 are for internal logic/debugging, not drawn if text is removed
        return
    end
    ...
    -- Determine focused track (first in-beam, in-range target)
    ...
    for i=1,#g_trks do local trk=g_trks[i];local in_r=(trk.dist>=g_mn_r and trk.dist<=g_mx_r)
        if in_r then local cur_trk_az_d=ad(trk.w_brg_d,r_w_d);local is_trk_bm=(math.abs(cur_trk_az_d)<=BEAM_W)
            if is_trk_bm then in_bm=true -- Set flag if any target is in beam
                if not g_fcs_trk then -- Select first valid target as focused
                    ...
                    -- g_d_t2 and g_d_t3 are for internal logic/debugging, not drawn if text is removed
    ...
    -- Update display text if no target is focused (internal logic only if not drawn)
    ...
    if g_fcs_trk then
        local eff_it = ECG_INT_T / s_anim_m  -- Effective ECG interval
        local eff_rt = ECG_RISE_T / s_anim_m -- Effective ECG rise time
        local eff_ft = ECG_FALL_T / s_anim_m -- Effective ECG fall time

        -- Reset heartbeat timer if interval has passed
        if g_t - g_fcs_trk.l_beat_t >= eff_it then
            g_fcs_trk.l_beat_t = g_t
            -- Update in main tracks list as well
            for k = 1, #g_trks do
        ...
        local el_b = g_t - g_fcs_trk.l_beat_t -- Elapsed time since last beat trigger
        -- Determine if the heartbeat *timer* is in its active phase (rise or fall)
        g_fcs_trk.is_beat = (el_b >= 0 and el_b < eff_rt + eff_ft) -- Flag for onDraw
    else
        if prev_fcs then prev_fcs.is_beat = false end -- Clear flag if focus was lost
    end

    -- Radar sweep speed and lock logic
    local d_spd=DEF_SPD
    if bLckH then -- Lock button held
        if in_bm then -- Target in beam
            if g_lck_st~=2 then g_srch_c_brg=r_w_d end -- Store current bearing if just entered lock
            g_lck_st=2;d_spd=0 -- State 2: Locked on target, stop sweep
        else -- Target not in beam (or lost)
            if g_lck_st==2 then -- Was locked, now lost
                g_lck_st=3 -- State 3: Re-acquiring
                if g_srch_c_brg then d_spd=DEF_SPD*SRCH_SPD_F*g_srch_dir else g_lck_st=1;d_spd=DEF_SPD end
            elseif g_lck_st==3 then -- Currently re-acquiring
                if g_srch_c_brg then d_spd=DEF_SPD*SRCH_SPD_F*g_srch_dir
                    -- Reverse search direction if arc limit reached
                    if math.abs(ad(r_w_d,g_srch_c_brg))>=SRCH_ARC_D/2 then g_srch_dir=g_srch_dir*-1;d_spd=DEF_SPD*SRCH_SPD_F*g_srch_dir end
                else g_lck_st=1;d_spd=DEF_SPD end -- Fallback if no search bearing
            else -- State 0 or 1 (Lock button pressed but no target yet)
                g_lck_st=1;d_spd=DEF_SPD -- State 1: Searching with lock active
            end
        end
    else -- Lock button not held
        g_lck_st=0;d_spd=DEF_SPD;g_srch_dir=1 -- State 0: Normal sweep
    end
    isN(O_RAD_SPD,d_spd)

    -- Set O_ECG_BEAT_OUT based on whether a peak was visually drawn in the *last* frame
    if g_pwr then
        isB(O_ECG_BEAT_OUT, g_peak_drawn_last_frame)
    end -- If not g_pwr, it was already set to false at the start of onTick

    -- Update general outputs (g_d_t1, g_d_t4 are for internal logic if not drawn)
...
-- Draw
function onDraw()
    -- Reset flag at the start of each draw cycle. It will be set true if a peak is drawn.
    g_peak_drawn_last_frame = false

    -- If radar is off, draw "RADAR OFF" message (Text removed)
    if not g_pwr then
        screen.setColor(100,100,100,255);screen.drawClear() -- Clear screen to gray
        -- screen.drawText(SCR_W/2-20,SCR_H/2-3,"RADAR OFF") -- Text removed
        -- screen.drawText(MRG,SCR_H-8,string.format("%.0f",g_mn_r)) -- Text removed
        -- local m_r_txt=string.format("%.0f",g_mx_r) -- Text removed
        -- local t_w=string.len(m_r_txt)*4 -- Text removed
        -- screen.drawText(SCR_W-MRG-t_w,SCR_H-8,m_r_txt) -- Text removed
        -- screen.drawText(1,19,g_d_t4) -- Text removed
        g_swp_drw_st = "sweeping"; g_pk_drw_inf = nil; -- Reset sweep state
        return
    end

    -- If focused track is lost while peaking, revert to sweeping
    if g_swp_drw_st == "peaking" then
        if not g_fcs_trk or (g_pk_drw_inf and g_fcs_trk.id ~= g_pk_drw_inf.id) then
            g_swp_drw_st = "sweeping"
            if g_pk_drw_inf then g_swp_x = g_pk_drw_inf.x_pos else g_swp_x = g_swp_x end -- Resume sweep from peak's center
            g_pk_drw_inf = nil
        end
    end

    local s_anim_m = math.max(0.01, ANIM_SPD_M)    -- Safe animation speed multiplier
    local swp_inc = BASE_SWP_S * s_anim_m          -- Actual sweep increment per frame
    local eff_rt_drw = ECG_RISE_T / s_anim_m       -- Effective ECG rise time for drawing
    local eff_ft_drw = ECG_FALL_T / s_anim_m       -- Effective ECG fall time for drawing
    local pk_dur = eff_rt_drw + eff_ft_drw         -- Total duration of peak animation

    local vis_swp_x = g_swp_x -- This will be the end-point for the main sweep line drawing

    screen.setColor(0,255,0,255) -- Set sweep line color

    -- Peaking State: Draw paused line and animate heartbeat peak
    if g_swp_drw_st == "peaking" and g_pk_drw_inf then
        ...
        vis_swp_x = pk_left_x -- Draw main line only up to the LEFT edge of the peak's base.
        screen.drawLine(MRG, BASE_Y, vis_swp_x, BASE_Y)

        -- Animate and draw the peak
        local el_b_drw = g_t - g_pk_drw_st_t -- Elapsed time in peak animation
        local pk_h = ECG_REST_H             -- Current peak height
        local pk_base_h = g_pk_drw_inf.base_h -- Base height of the peak for this target

        if el_b_drw >= 0 then
            if el_b_drw < eff_rt_drw then -- Rise phase
                local p = el_b_drw / math.max(1, eff_rt_drw) -- Progress (0-1)
                pk_h = ECG_REST_H + p * (pk_base_h + ECG_ADD_H - ECG_REST_H)
            elseif el_b_drw < pk_dur then -- Fall phase
                local pf = (el_b_drw - eff_rt_drw) / math.max(1, eff_ft_drw) -- Progress (0-1)
                pk_h = (pk_base_h + ECG_ADD_H) - pf * (pk_base_h + ECG_ADD_H - ECG_REST_H)
            end
        end
        pk_h = cl(pk_h, ECG_REST_H, MAX_PK_H) -- Clamp peak height

        -- Draw the peak if it's above resting height
        if pk_h > ECG_REST_H then
            local peak_draw_xl = pk_left_x -- Left foot of the peak
            local peak_draw_xr = cl(pk_center_x + ECG_W / 2, MRG, SCR_W - MRG - 1) -- Right foot
            local yt = cl(BASE_Y - pk_h, 0, SCR_H - 1) -- Top Y of the peak
            screen.drawLine(peak_draw_xl, BASE_Y, pk_center_x, yt) -- Left slope
            screen.drawLine(pk_center_x, yt, peak_draw_xr, BASE_Y) -- Right slope
            g_peak_drawn_last_frame = true -- Set flag: peak was visually drawn
        end

        -- State transition logic: If peak animation is done, switch back to sweeping
        if el_b_drw >= pk_dur then
            g_swp_x = pk_center_x -- Logical sweep resumes from peak CENTER
            g_swp_drw_st = "sweeping"
            g_pk_drw_inf = nil
        end

    -- Sweeping State: Advance sweep line, check for transition to peaking
    else 
        local prev_swp_log_x = g_swp_x     -- Logical sweep position from previous frame (at start of this onDraw)
        g_swp_x = g_swp_x + swp_inc        -- Advance logical sweep position for current frame

        local to_peak_st = false           -- Flag to transition to peaking state
        local pk_tgt_x = 0                 -- X-coordinate of target's center to peak at

        -- Check if focused track exists, is beating, and sweep line is crossing it
        if g_fcs_trk and g_fcs_trk.is_beat then -- g_fcs_trk.is_beat is set by onTick timer
            pk_tgt_x = g_fcs_trk.x_pos -- Center of the peak
            -- Condition to trigger peak:
            -- If the sweep line's previous position was at or before the target's center,
            -- AND its new position is at or after the target's center.
            if prev_swp_log_x <= pk_tgt_x and g_swp_x >= pk_tgt_x then
                 if pk_tgt_x >= MRG and pk_tgt_x <= SCR_W - MRG then -- Target center is on screen
                    to_peak_st = true
                end
            end
        end

        if to_peak_st then -- Transition to peaking state
            g_swp_drw_st = "peaking"
            g_pk_drw_inf = { x_pos = pk_tgt_x, base_h = g_fcs_trk.base_h, id = g_fcs_trk.id }
            g_pk_drw_st_t = g_t
            g_swp_x = pk_tgt_x -- Align logical sweep TO the peak's center for the pause
            
            -- For this transitional frame, draw line up to the peak's left edge
            local pk_left_x_trans = cl(pk_tgt_x - ECG_W / 2, MRG, SCR_W - MRG - 1)
            vis_swp_x = pk_left_x_trans 
        else -- Continue normal sweep
            -- Wrap sweep line if it goes off screen
            if g_swp_x > (SCR_W - MRG) + swp_inc / 2 then
                g_swp_x = MRG
            end
            vis_swp_x = cl(g_swp_x, MRG, SCR_W - MRG) -- Clamp visual sweep to screen
        end
        screen.drawLine(MRG, BASE_Y, vis_swp_x, BASE_Y) -- Draw the sweep line
    end

    -- Draw common elements: range markers, text displays
    screen.drawLine(MRG,BASE_Y-2,MRG,BASE_Y+2) -- Left range marker
    -- Right range marker (only if sweep reaches end and not paused)
    if vis_swp_x >= SCR_W-MRG and g_swp_drw_st == "sweeping" then
        screen.drawLine(SCR_W-MRG,BASE_Y-2,SCR_W-MRG,BASE_Y+2)
    end

    -- Text drawing calls removed
